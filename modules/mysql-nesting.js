...
exports.convertToNested = function (rows, nestingOptions) {
	...
	for(var r=0; r<rows.length; r++) {
		if(rows[r]['']) {
			rows[r].social = {
				socialId: rows[r].courses.courseId,
				courseId: rows[r].courses.courseId
			};
			for(var l=0; l<Object.keys(rows[r]['']).length; l++) {
				rows[r].social[Object.keys(rows[r][''])[l]] = rows[r][''][Object.keys(rows[r][''])[l]];
			}
		}
	}
